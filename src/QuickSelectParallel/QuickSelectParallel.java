...
public class QuickSelectParallel
{
	...
	public static void main(String[] args)
	{
		...
		List<BlockingQueue<Object>> inPutQueues = new ArrayList<>(numClients);
		for(int i = 0; i < numClients; i++)
		{
			outPutQueues.add(new ArrayBlockingQueue<>(1));
			inPutQueues.add(new ArrayBlockingQueue<>(1));
		}

		Thread[] threads = new Thread[numClients + 1];

		// Server Thread
		threads[0] = new Thread(new Server(inPutQueues, outPutQueues, arrayToSort, numClients, k));

		// Client Threads, take from respective Queue, put on Q1
		for(int i = 1; i < threads.length; i++)
		{
			threads[i] = new Thread(new Client(outPutQueues.get(i - 1), inPutQueues.get(i - 1)));
		}
